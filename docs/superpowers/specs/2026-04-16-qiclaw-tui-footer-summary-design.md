...
- Summary của turn ngắn gọn nhưng đủ thông tin để người dùng biết kết quả turn, verification, số lượt gọi provider, số tool calls, và tổng thời gian.
...
- `turn_summary_text`: summary ngắn gọn của turn gần nhất, ví dụ `completed • verified • 1 provider • 2 tools • 18s`
...
`completed • verified • 1 provider • 2 tools • 18s`
...
- Không đưa token counts, model name, hay path vào turn summary vì model đã ở cột phải và các thông tin khác không cần thiết cho mục tiêu hiện tại.
...
1. format `completed • verified • 1 provider • 2 tools • 18s`
2. singular/plural đúng cho `provider/providers` và `tool/tools`
3. duration dưới 1 giây hiển thị theo `ms`
4. `max_tool_rounds_reached` được rút gọn thành `max tools`
5. tool count là số calls thực tế, không phải tool rounds
