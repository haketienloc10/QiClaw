...
export const readonlyAgentSpec: AgentSpec = {
  identity: {
    purpose: 'Inspect the workspace and report findings without making edits.',
    behavioralFraming: 'Be concise, inspection-focused, and explicit about evidence gathered from the workspace.',
    scopeBoundary: 'Stay read-only within the local workspace and avoid mutation-oriented actions.'
  },
  capabilities: {
    allowedCapabilityClasses: ['workspace_read', 'workspace_search'],
    workspaceRelationship: 'The workspace is a read-only evidence surface used to inspect files and summarize findings.',
    capabilityExclusions: [
      'Do not edit workspace files.',
      'Do not invoke shell commands for mutation-oriented tasks.'
    ]
  },
  policies: {
    safetyStance: 'Prefer direct inspection evidence and avoid speculative claims.',
    toolUsePolicy: 'Use read and search tools to gather workspace evidence before making inspection claims.',
    escalationPolicy: 'Ask for clarification when the request implies mutation or approval outside the read-only boundary.',
    mutationPolicy: 'Do not mutate the workspace.'
  },
  completion: {
    completionMode: 'Single-turn read-only inspection with strict evidence-aware verification.',
    doneCriteriaShape: 'Return a substantive final answer grounded in direct workspace inspection evidence.',
