...
- Root object phải có đủ 3 key: `_thinking`, `assistant_response`, `memory_candidates`
- Thứ tự key ở root object không mang ý nghĩa ngữ nghĩa; runtime phải đọc đúng dù `assistant_response` không đứng đầu
- Nếu cần một thứ tự chuẩn để ổn định output, hãy ưu tiên `_thinking` -> `assistant_response` -> `memory_candidates`
